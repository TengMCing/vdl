...
#define assert_IncompatibleType(type1, type2) expect(type1 == type2, printf("ERROR: Incompatible types (%d != %d) provided!", type1, type2))
#define assert_ZeroCapacity(capacity) expect(capacity > 0, printf("ERROR: Zero or negative capacity [%d] provided!", capacity))
#define assert_ZeroObjects(num_object) expect(num_object > 0, printf("ERROR: Zero or negative objects [%d] provided!", num_object))
